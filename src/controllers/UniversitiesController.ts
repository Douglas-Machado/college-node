...
class UniversitiesController{
  ...
  async createUniversity(request: Request, response: Response){
		try{
			const { alpha_two_code, web_pages, name, country, domains, stateProvince } = request.body
			
			const result = await service.create({alpha_two_code, web_pages, name, country, domains, stateProvince})
